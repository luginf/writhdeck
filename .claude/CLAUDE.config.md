...
**Subfolder navigation (`[behaviour]`)** — `browser_subdirs` (boolean, default `yes`) enables explorer-style browsing of subdirectories. Runtime gate for the optional `src/subdirs.tcl` module; ignored when that module is absent (mini/jim builds). See `.claude/CLAUDE.browser.md`.
