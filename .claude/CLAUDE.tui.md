...
- `tui-repetitions-dialog {fpath rows cols}` — repetition-check overlay, `q` to close. Computed lazily: only reachable via the `r` key from `tui-analyse-dialog` (GUI equivalent: "Repetitions" button in `analyse-dialog`), never run as part of the normal Structure Analysis pass, to avoid the extra cost on every `a` invocation. Uses shared `find-repetitions {fpath}` (`src/common.tcl`) — two tiers: same word/lemma repeated within `cfg_repetition_scope` words (always on), and hidden-substring repetition like "tour" inside "alentours" (optional, `cfg_repetition_hidden`, filtered by `cfg_repetition_min_len` and `repetition-stopwords`).
