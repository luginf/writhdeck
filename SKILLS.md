...
- Format `daily` : une entrée par fichier `"filepath\tdate1\tN1\tdate2\tN2..."` ; le `\t` est la séquence JSON d'échappement (deux caractères `\t`), pas un tab littéral (invalide en JSON). `state-parse-array` utilise la regex `(?:[^"\\]|\\.)*` pour gérer les séquences échappées ; `state-load` applique `string map [list {\\t} "\t"]` avant le `split`.
