...
Accessible via la touche `c` dans le browser. Invoqué par la proc `profile-config-dialog`. Quatre onglets : **Profile**, **Timer**, **Misc**, **Display**.

- **Profile** : dropdown profil actif + scheme + langue ; police (listbox+entry), taille, marges, objectif mots, mode sombre, interligne (%), hauteur barre, numéros de ligne, curseur bloc, curseur clignotant
...
Configuration par profil stockée dans `::cfg_profiles` (clés : `font_family`, `font_size`, `margin_width`, `margin_height`, `word_goal`, `dark_mode`, `line_spacing`, `bar_height`, `line_numbers`, `block_cursor_gui`, `blink_cursor`). Chargement via `profile-config-update-profile {w}` au démarrage et sur changement de profil (trace).
