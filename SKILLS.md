...
Accessible via la touche `c` dans le browser. Invoqué par la proc `profile-config-dialog`. Cinq onglets : **Profile**, **Fonts**, **Timer**, **Misc**, **Display**.

- **Profile** : dropdown profil actif + scheme + langue ; marges, objectif mots, mode sombre, interligne (%), hauteur barre, numéros de ligne, curseur bloc, curseur clignotant
- **Fonts** : dropdown profil (lié à la même variable `::profile_config_profile` que le Profile tab) ; entry famille de police ; listbox polices disponibles + scrollbar ; label preview (initialisé avec la police et taille courantes à l'ouverture) ; spinbox taille
- **Timer** : type (countdown/stopwatch), durée (spinbox), son, alerte, afficher dans barre
- **Misc** : Autosave (activé/intervalle) ; Behaviour : dossier documents (entry + bouton Parcourir), navigateur au démarrage, surveiller modifications, mode Hemingway, réduire marge en split, restaurer position curseur
- **Display** : zones barre de statut (gauche/centre/droite) ; marqueur de titre ; section Markup : marqueurs commentaire/gras/italique/souligné/barré, titres Markdown

**Module optionnel `src/gui-config.tcl`** : contient les trois procs du dialogue (`profile-config-dialog`, `config-tab-switch`, `profile-config-update-profile`), enveloppées dans `if {!$::no_gui}`. Exclu avec `make GUI_CONFIG=no` (~700 lignes économisées). La touche `c` et le bind clavier sont conditionnels via `[info procs profile-config-dialog] ne ""` — vrai seulement si le module a été inclus dans le build.

Configuration par profil stockée dans `::cfg_profiles` (clés : `font_family`, `font_size`, `margin_width`, `margin_height`, `word_goal`, `dark_mode`, `line_spacing`, `bar_height`, `line_numbers`, `block_cursor_gui`, `blink_cursor`). Chargement via `profile-config-update-profile {w}` au démarrage et sur changement de profil (trace). La proc met aussi à jour le label preview de la Fonts tab via `catch {$w.tab_fonts.preview configure -font ...}` (catch car le widget peut ne pas exister).

Détails clés :
- Les deux dropdowns de profil (Profile tab et Fonts tab) partagent la même variable `::profile_config_profile` — changer l'un synchronise l'autre
- `-command` sur spinbox pour mettre à jour le preview sur clic des boutons (pas juste clavier)
- `marker-val` appliqué à la lecture et à la sauvegarde des marqueurs (affiche `""` pour les marqueurs désactivés)
- Bouton Parcourir → `tk_chooseDirectory`, stocke le chemin avec substitution `~` pour le HOME
- Dialog détruite après fermeture par l'utilisateur
- `gui-config.tcl` doit être inclus AVANT `gui.tcl` dans le build (le code de setup des raccourcis dans gui.tcl s'exécute au moment du parse et vérifie `[info procs]`)
...
## Récemment implémenté (session 2026-06-10)

- **Filtre d'extensions du browser** : nouvelle option `browser_filter` dans `[behaviour]` — liste de motifs glob séparés par des espaces (défaut `*.txt *.t2t *.md *.ini`), appliquée par `list-docs` (`src/common.tcl`) via `string match -nocase` sur chaque nom de fichier (vide = tout afficher). Nouvelle option `browser_show_all` (défaut `no`) bypass complètement le filtre. Les deux sont éditables dans l'onglet Misc (entrée `ffilter` + checkbox `fshowall`, première de la boucle de checkboxes booléennes). Favoris/Récents (`build-extra-entries`) ne sont pas filtrés — seul le listing du dossier principal l'est. Traduit dans les 6 langues (`config_browser_filter`, `config_browser_show_all`).

- **Raccourci TOC épinglé (Shift+Ctrl+F11)** : nouvelle clé `key_toc_pinned` dans `[keys]` (défaut `Control-Shift-F11`). Nouvelle proc `toc-panel-toggle` (factorisée depuis la logique d'ouverture/fermeture du panneau `.ed.toc`) bascule le panneau épinglé quel que soit l'état de `cfg_toc_pinned` — alors que F11 (`cfg_key_toc`) bascule entre popup et panneau épinglé selon `cfg_toc_pinned`. Bindé sur `.ed.t`, chaque pane `split-make-pane`, le pane WS2 (`.ed.pw.r.t`) et `.ed.toc.lst` (ferme le panneau). GUI uniquement (absent de `writhdeck-cli.tcl`).
